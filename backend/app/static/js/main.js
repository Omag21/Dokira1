...
function registerPWA() {
    ...
    window.addEventListener('beforeinstallprompt', (event) => {
        // Ne pas bloquer le banner natif pour éviter l'avertissement console.
        deferredInstallPrompt = event;
    });
}

async function setupTelechargementButton() {
    const button = document.getElementById('telechargementBtn');
    if (!button) return;

    button.addEventListener('click', async (event) => {
        event.preventDefault();

        const apkUrl = button.getAttribute('data-apk-url');
        if (apkUrl) {
        ...
        if (isIos) {
            alert("Sur iPhone/iPad: ouvrez le menu Partager puis 'Sur l’écran d’accueil' pour installer Dokira.");
            return;
        }

        alert("L’installation directe n’est pas disponible pour le moment. Ouvrez Dokira dans Chrome/Edge mobile puis utilisez 'Installer l’application'.");
    });
}

// ============================================
// CHAT IA - PAGE D'ACCUEIL
// ============================================
function setupHomeAssistant() {
    if (document.getElementById('homeAiWidget')) return;

    const widgetHtml = `
        <button id="homeAiFab" class="home-ai-fab" aria-label="Ouvrir l'assistant IA">
            <i class="fas fa-brain"></i>
        </button>
        <div id="homeAiWidget" class="home-ai-widget">
            <div class="home-ai-header">
                <span>Assistant IA Dokira</span>
                <button id="homeAiClose" type="button"><i class="fas fa-times"></i></button>
            </div>
            <div id="homeAiMessages" class="home-ai-messages">
                <div class="home-ai-msg ia">Bonjour, je peux répondre à vos questions générales de santé et sur Dokira.</div>
            </div>
            <form id="homeAiForm" class="home-ai-form">
                <input id="homeAiInput" type="text" placeholder="Posez votre question..." required />
    ...
    document.body.insertAdjacentHTML('beforeend', `
        <style>
            .home-ai-fab{position:fixed;right:18px;bottom:18px;width:56px;height:56px;border:none;border-radius:50%;background:#0D8ABC;color:#fff;z-index:1100;box-shadow:0 8px 20px rgba(13,138,188,.35)}
            .home-ai-widget{position:fixed;right:18px;bottom:84px;width:340px;max-width:calc(100vw - 24px);height:460px;background:#fff;border:1px solid #e5e7eb;border-radius:14px;display:none;flex-direction:column;z-index:1100}
            .home-ai-widget.open{display:flex}
            .home-ai-header{display:flex;justify-content:space-between;align-items:center;padding:10px 12px;background:#0D8ABC;color:#fff;border-radius:14px 14px 0 0}
            .home-ai-header button{background:transparent;border:none;color:#fff}
            .home-ai-messages{flex:1;overflow:auto;padding:12px;background:#f8fafc}
            .home-ai-msg{max-width:88%;padding:8px 10px;border-radius:10px;margin-bottom:8px;font-size:.92rem;color:#1e293b}
            .home-ai-msg.ia{background:#fff;border:1px solid #e2e8f0;color:#1e293b}
            .home-ai-msg.user{margin-left:auto;background:#0D8ABC;color:#fff}
            .home-ai-form{display:flex;gap:8px;padding:10px;border-top:1px solid #e5e7eb}
            .home-ai-form input{flex:1;border:1px solid #cbd5e1;border-radius:8px;padding:8px 10px}
            .home-ai-form button{border:none;border-radius:8px;padding:0 12px;background:#0D8ABC;color:#fff}
            @media (max-width: 576px){.home-ai-widget{height:58vh;right:10px;bottom:76px}.home-ai-fab{right:10px;bottom:12px}}
        </style>
    `);

    const fab = document.getElementById('homeAiFab');
    const widget = document.getElementById('homeAiWidget');
    const close = document.getElementById('homeAiClose');
    ...
    const addMessage = (content, role) => {
        ...
        messages.insertAdjacentHTML('beforeend', `<div class="home-ai-msg ${role}">${escaped}</div>`);
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            const iaText = data.ia_response || "Je n'ai pas pu générer de réponse.";
            addMessage(iaText, 'ia');
            history.push({ role: 'assistant', content: iaText });
            if (data.ai_source === 'fallback' && data.ai_error) {
                addMessage(`Info API: ${data.ai_error}`, 'ia');
            }
        } catch (error) {
            addMessage("Une erreur est survenue. Réessayez dans quelques instants.", 'ia');
            console.error('Erreur assistant IA accueil:', error);
        }
    });
}